...
function ProductsTable(props) {
    ...
    if (searchText.length !== 0) {
      setData(
        _.filter(products, (item) =>
          item?.institutionName?.toLowerCase().includes(searchText.toLowerCase())||item?.institutionBranchName?.toLowerCase().includes(searchText.toLowerCase())||item?.gateWayName?.toLowerCase().includes(searchText.toLowerCase()),
        ),
      )
      setPage(0)
    } else {
  ...
  function handleClick(item) {
    ...
    props.navigate(`/auth_options/${item.id}`)
  }
