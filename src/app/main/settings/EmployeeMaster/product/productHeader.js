...
import { searchFilter } from '../store/productsSlice'
...
function ProductHeader(props) {
  ...
  async function handleSaveProduct() {
    try {
      ...
      let actionResult;
      if (id == "new") {
        // If there's no ID, save a new product
        actionResult = await dispatch(saveProduct(params));
      } else {
        // Otherwise, update the existing product
        actionResult = await dispatch(updateProduct({ ...params }));
      }
      ...
      dispatch(searchFilter({val:searchText ? searchText : "" }));
    } catch (err) {
